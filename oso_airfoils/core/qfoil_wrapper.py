"""
qfoil_wrapper.py  --  oso's adapter over metafoil's Qfoil (RFOIL).
...
oso's historical call signature and return contract:
  ...
  * alpha mode goes to metafoil's in-memory Qfoil (``libqfoil.so`` via f2py) --
    no temp files, ~10x faster for sweeps;
  * CL mode, flap deflections and an explicitly-pointed qfoil binary go to
    metafoil's subprocess/file-I/O Qfoil, which additionally does the NeuralFoil
...
alias for ``N_panels_kulfan``, and the ``N_panels`` / ``N_panels_qfoil`` keys that
...
def run(mode,
        ...
        **kwargs):
    """
    Run Qfoil for a Kulfan airfoil.
    ...
    if needs_fileio:
        res = _qfoil_fileio.run_from_kulfan(
