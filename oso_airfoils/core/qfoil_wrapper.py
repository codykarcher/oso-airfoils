...
import math
import shutil
# path_to_QFOIL_default = shutil.which('qfoil')
import pathlib
import random
import string
...
def _nf_cl_check(target_cl, upperKulfanCoefficients, lowerKulfanCoefficients,
    ...
    alpha estimate for Qfoil inviscid seeding.

    NeuralFoil is required for Qfoil CL mode.  An ImportError is raised if
    it is not installed.
...
def run(mode, 
    ...
    # Fall back to the project-relative binary when qfoil is not on PATH.
    # The wrapper lives at  oso_airfoils/core/qfoil_wrapper.py, so
    # path_to_here.parent.parent  is the workspace root (oso-airfoils/).
    if path_to_QFOIL_default is None:
        _project_bin = path_to_here.parent.parent / 'Qfoil' / 'bin' / 'qfoil'
        if _project_bin.exists():
            path_to_QFOIL_default = str(_project_bin)

    if path_to_QFOIL is None:
        path_to_QFOIL = path_to_QFOIL_default
    ...
    if mode == 'cl':
        ...
        if not _nf_achievable:
            warnings.warn(
                f"Skipping Qfoil: CL={_cl_seed:.4f} exceeds {cl_margin}×"
    ...
    if N_panels_qfoil is not None and N_panels_kulfan is not None:
        warnings.warn(
            f"Both N_panels_qfoil={N_panels_qfoil} and N_panels_kulfan={N_panels_kulfan} "
            "are set.  qfoil will repanel after loading the Kulfan-generated dat file.",
            stacklevel=2,
        )
    ...
    try:
        ...
        if mode == 'cl' and (data.ndim == 0 or data.size == 0):
            warnings.warn(
                f"Qfoil returned no converged solution for CL={_cl_seed:.4f} "
        ...
        if not is_iterable:
            ...
            if mode == 'cl' and _nf_alpha_est is not None and not np.isnan(alpha):
                ...
                if abs(float(alpha) - _nf_alpha_est) > _alpha_tol:
                    warnings.warn(
                        f"Qfoil alpha={float(alpha):.2f}° deviates from NeuralFoil "
        ...
        res['M'] = Mval
        res['N_crit'] = N_crit
        res['N_panels_qfoil'] = N_panels_qfoil
        ...
        if stdout_log_path is not None and os.path.exists(tempStdoutFile.name):
            ...
            if os.path.isdir(stdout_log_path):
                ...
                _fname = f'qfoil_stdout_{_name_part}Re{Re:.0f}_Ncrit{N_crit:.1f}_xtp{xtp_u:.2f}_{xtp_l:.2f}.txt'
        ...
        if exec_script_path is not None and os.path.exists(tempExecFile.name):
            ...
            if os.path.isdir(exec_script_path):
                _name_part = f'{airfoil_name}_' if airfoil_name else ''
                _fname = f'qfoil_exec_{_name_part}Re{Re:.0f}_Ncrit{N_crit:.1f}_xtp{xtp_u:.2f}_{xtp_l:.2f}.txt'
