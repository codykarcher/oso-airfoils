...
# metafoil's in-memory XFOIL (libxfoil.so via f2py) — the default solve path.
...
import metafoil.xfoil as _metafoil_xfoil
...
def run(mode,
    ...
    Alpha mode is served by metafoil's in-memory XFOIL (libxfoil.so) — no temp
    files, ~10x faster for sweeps, results validated against the xfoil CLI.
    The signature and return contract are unchanged, so every existing call
    site is unaffected.
    ...
    (:func:`_run_fileio`, which drives the xfoil binary via temp files).
    ...
    # Features the in-memory solver doesn't cover -> proven file-I/O path.
    _needs_fileio = (m == 'cl'
    ...
    if _needs_fileio:
        ...
        res = _metafoil_xfoil.run_from_kulfan(
    ...
    # Reconcile return keys with the oso contract / core.sweep consumer,
    # which reads res['N_panels'] and (historically) res['N_panels_xfoil'].
