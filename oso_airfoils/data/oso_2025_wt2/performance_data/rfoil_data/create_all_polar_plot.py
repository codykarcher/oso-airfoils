...
if rank == 0:
    ...
    if 'released_designs' in str(path_to_here):
        for tix,t in enumerate(['21','24','27','30','33','36']):
            rfoil_comparison['OSO_'+t] = { 'clean':{ 'drag_on':{}, 'drag_off':{} }, 'rough':{ 'drag_on':{}, 'drag_off':{} } , 'color': colors[tix]      ,'path':path_to_oso+'/released_designs/OSO_2025_WT2', 'name': 'OSO_2025_WT2_T'+t }

    for ix, kwd in enumerate(rfoil_comparison.keys()):
        tau = kwd[-2:]
        path_to_data = rfoil_comparison[kwd]['path']+'/rfoil_data'
    ...
    plt.savefig(str(path_to_here) + os.sep + 'CL_v_CD_clean_combined.png', dpi=250)
    plt.close()
    ...
    plt.tight_layout()
    plt.legend(loc='lower right', framealpha=1)
    plt.savefig(str(path_to_here) + os.sep + 'CL_v_CD_rough_combined.png', dpi=250)
    plt.close()