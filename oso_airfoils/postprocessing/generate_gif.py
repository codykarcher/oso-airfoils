"""Generate an animated GIF showing the evolution of the Pareto front across
optimization generations.

Usage (from any directory):
    mpirun -n 8 python -m oso_airfoils.postprocessing.generate_gif [path_to_data]
                       [--output OUTPUT] [--frames-dir FRAMES_DIR]
                       [--dpi DPI] [--duration DURATION]

If *path_to_data* is omitted the current working directory is used, so you can
simply ``cd`` into a case run folder and call ``oso-gif``.

Example:
    cd cases_111_to_120/case_116/c116_t18_k16_n752_l13_e15__2026_06_15_10-30-00
    oso-gif
"""
...
# ── Argument parsing (identical on every MPI rank) ───────────────────────────
parser = argparse.ArgumentParser(
    description='Generate a GIF of Pareto-front evolution from optimizer output.'
)
parser.add_argument(
    'path_to_data',
    nargs='?',
    default='.',
    help='Path to the case run directory (default: current working directory).',
)
parser.add_argument(
    '--output', '-o',
    default='pareto_shapes_evolution.gif',
    dest='output_gif_name',
    help='Output GIF filename (default: pareto_shapes_evolution.gif).',
)
parser.add_argument(
    '--frames-dir',
    default='_combined_gif_frames',
    dest='frames_dir_name',
    help='Subdirectory name for temporary frame images (default: _combined_gif_frames).',
)
parser.add_argument(
    '--dpi',
    type=int,
    default=100,
    dest='image_dpi',
    help='Frame image DPI (default: 100).',
)
parser.add_argument(
    '--duration',
    type=int,
    default=100,
    dest='frame_duration',
    help='Milliseconds per frame in the output GIF (default: 100).',
)
args = parser.parse_args()

path_to_data   = os.path.abspath(args.path_to_data)
output_gif_name = args.output_gif_name
frames_dir_name = args.frames_dir_name
image_dpi       = args.image_dpi
frame_duration  = args.frame_duration

assert os.path.isdir(path_to_data), \
    f"path_to_data is not a valid directory: '{path_to_data}'"
...
# xlim_bottom is always fixed; all other limits are derived from the last file.
xlim_bottom = (-0.05, 1.05)

# ── Auto-limit tuning factors ─────────────────────────────────────────────────
xlim_top_factor    = 1.40
ylim_top_factor    = 1.20
xlim_pareto_factor = 2.00
ylim_pareto_factor = 1.60
ylim_bottom_factor = 1.40
...
fix_colormap_range = None   # (min_val, max_val) or None for auto
font_size          = 20
legend_font_size   = 12
...
for ix, gen_i in enumerate(gen_indices):
    ...
    # Sample evenly along arc length of the Pareto front
    ...
    # Build airfoil shapes for this generation
    shape_curves = []
    for i in list(reversed(ixs)):
        ...
        if fix_colormap_range is not None:
            ...
            clamped     = np.clip(rough_ld_value, cmap_min, cmap_max)
            color_idx   = int(np.clip((clamped - cmap_min) / (cmap_max - cmap_min) * (nafl - 1), 0, nafl - 1))
        else:
    ...
    # Left: full population + highlighted Pareto
    pareto_indices = np.sort(np.unique([p['pareto_index'] for p in pop]))
    for pix in list(reversed(pareto_indices)):
        pts  = sorted([p for p in pop if p['pareto_index'] == pix], key=lambda p: p['LoD_rough_at_design'])
    ...
    pareto_points = sorted([p for p in pop if p['pareto_index'] == 1], key=lambda p: p['LoD_rough_at_design'])
    rough_LD_vals = np.array([p['LoD_rough_at_design'] for p in pareto_points])
    clean_LD_vals = np.array([p['LoD_clean_at_design'] for p in pareto_points])
    ...
    for i in ixs:
        ...
        if fix_colormap_range is not None:
            ...
            color_idx = ixs.tolist().index(i)
        ax_left.plot(rough_ld_value, clean_LD_vals[i], 'o', color=turbo_cmap(color_idx), ms=9)
    ...
    # Top: Kulfan parameters vs rough L/D
    cmap_counter = 0
